...
public class UniDimmer_NKR extends ioResource implements IHCResource,java.io.Serializable, OnSeekBarChangeListener {
	...
	public void setupResources(XmlPullParser xpp, String endTag) {
		...
		try 
		{
			...
			while(tagname != endTag)
			{
				...
				{
					this.touchId = Integer.parseInt(xpp.getAttributeValue(null, "id").replace("_0x", ""), 16);			
				}	
	...
	public void setInputClicked(boolean OnOff, int inputID, ConnectionManager ihcCtrl) {
		...
		request.addProperty("typeString", "airlink_dimmer_touch");
		request.addProperty("isValueRuntime", "true");
